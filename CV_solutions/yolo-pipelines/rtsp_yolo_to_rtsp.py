...
class RTSPPublisher:
    """
    Envía frames BGR por stdin a FFmpeg para publicar H.264 en RTSP (MediaMTX).
    Perfil low-latency por encoder.
    """
    def __init__(self, rtsp_url: str, size: Tuple[int,int], fps: int, bitrate: str,
                 encoder: str = "auto", suggest_nvenc=False, suggest_jetson=False, gop: int = 25):
        ...
        if encoder == "auto":
            if suggest_nvenc:   encoder = "nvenc"
            elif suggest_jetson: encoder = "jetson"
            else:               encoder = "cpu"

        if encoder == "nvenc":
            enc = f"-c:v h264_nvenc -preset p3 -tune ll -rc cbr_ld_hq -b:v {bitrate} -maxrate {bitrate} -bufsize {bitrate} -g {self.gop} -bf 0"
        elif encoder == "jetson":
            # Muchas builds usan también h264_nvenc en Jetson actual; si tu FFmpeg tiene nvv4l2h264enc, cámbialo:
            enc = f"-c:v h264_nvenc -preset p3 -tune ll -rc cbr_ld_hq -b:v {bitrate} -maxrate {bitrate} -bufsize {bitrate} -g {self.gop} -bf 0"
        else:  # CPU libx264
            # zerolatency + GOP fijo + sin B-frames
            enc = (f"-c:v libx264 -preset veryfast -tune zerolatency -b:v {bitrate} "
                   f"-maxrate {bitrate} -bufsize {bitrate} -g {self.gop} -bf 0 "
                   f"-x264-params scenecut=0:open_gop=0:min-keyint={self.gop}:keyint={self.gop}")

        # IMPORTANTE: sin -re (no throttlear); timestamps por rtp los maneja FFmpeg
        cmd = (
            f"ffmpeg -loglevel error "
            f"-f rawvideo -pix_fmt bgr24 -s {self.w}x{self.h} -r {self.fps} -i - "
            f"-vf format=yuv420p {enc} "
            f"-f rtsp -rtsp_transport tcp {shlex.quote(self.rtsp_url)}"
        )
        self.proc = subprocess.Popen(shlex.split(cmd), stdin=subprocess.PIPE)

    def write(self, frame_bgr: np.ndarray):
        """Escribe un frame (BGR) a stdin de FFmpeg."""
