...
class RTSPPublisher:
    """
    Publica frames por RTSP usando GStreamer (rtspclientsink).
    En Jetson intenta nvv4l2h264enc (HW); si no, cae a x264enc (CPU).
    """
    def __init__(self, rtsp_url, size, fps=25, bitrate="4M",
        ...
        kbps = int(bitrate[:-1]) * 1000 if bitrate.lower().endswith("m") else 4000

        # 1) Intento Jetson HW
        if encoder == "jetson" or suggest_jetson:
            pipeline = (
                "appsrc ! "
                "videoconvert ! "                  # si tienes nvvideoconvert, puedes usarlo en su lugar
                f"video/x-raw,format=I420,framerate={fps}/1,width={self.w},height={self.h} ! "
                f"nvv4l2h264enc iframeinterval={gop} idrinterval={gop} control-rate=1 bitrate={kbps} preset-level=1 insert-sps-pps=true ! "
                "h264parse config-interval=1 ! "
                f"rtspclientsink location={rtsp_url} protocols=tcp"
            )
            self.out = cv2.VideoWriter(pipeline, cv2.CAP_GSTREAMER, 0, float(fps), (self.w, self.h))

        # 2) Fallback CPU con x264 (si lo anterior no abrió)
        if (self.out is None) or (not self.out.isOpened()):
            pipeline = (
                "appsrc ! videoconvert ! "
                f"video/x-raw,format=I420,framerate={fps}/1,width={self.w},height={self.h} ! "
                f"x264enc tune=zerolatency key-int-max={gop} bframes=0 bitrate={kbps} speed-preset=veryfast ! "
                "h264parse config-interval=1 ! "
                f"rtspclientsink location={rtsp_url} protocols=tcp"
            )
            self.out = cv2.VideoWriter(pipeline, cv2.CAP_GSTREAMER, 0, float(fps), (self.w, self.h))
        ...
        if not self.out.isOpened():
            raise RuntimeError("No se pudo abrir GStreamer VideoWriter. Revisa plugins (nvv4l2h264enc/x264enc) y pipeline.")
