...
def infer_age_gender(face_bgr, age_net, gender_net):
    ...
    Modelos tipo ImageNet (224x224, RGB, mean/std: 0.485/0.456/0.406, 0.229/0.224/0.225).
    """
    ...
    blob = cv2.dnn.blobFromImage(
        ...
        size=(224, 224),
        mean=(0, 0, 0),
        swapRB=True,         # <-- MUY IMPORTANTE (BGR->RGB)
        crop=False
    )
...
def run_tracker_and_publish(src, publish_name, source_index):
    ...
    try:
        for r in model.track(
            ...
            show=False,
            verbose=False,
            persist=False,               # IDs consistentes
            ...
            if boxes is not None and len(boxes) > 0:
                ...
                for i in range(len(xyxy)):
                    ...
                    # --- Recorta cara de forma segura
                    x1c, y1c = max(0, x1i), max(0, y1i)
                    x2c, y2c = min(w_img, x2i), min(h_img, y2i)
                    ...
                    if need_update and object_id != -1 and (x2c > x1c and y2c > y1c):
                        ...
                        try:
                            ...
                            # si falla, conserva últimos valores si existen
                            pass

                    # --- Si no se actualizó en este frame, toma del caché
